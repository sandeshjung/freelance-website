...
import SendRatings from '../User/SendRatings'
import RequestJob from '../User/RequestJob';
...
function Recommend() {
    ...
    // useEffect(async()=>{
    {
       await axios.get(`http://localhost:5000/api/recommend/`)
        .then(resp => {
  ...
  return (
    ...
    <div>
       ...
       <button onClick={handleClick} className="btn btn-outline-success">Show Recommended jobs</button>
        </div>
        <div>
        <Tabs
                                value={value}
